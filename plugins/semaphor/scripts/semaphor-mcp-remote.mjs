...
import {
  DATA_APP_AUTHORING_BOOTSTRAP_TOOL_DEFINITIONS,
  assertCanonicalDataAppAuthoringDefinitions,
...
const FALLBACK_TOOLS = DATA_APP_AUTHORING_BOOTSTRAP_TOOL_DEFINITIONS.map((tool) => ({
  ...tool,
...
function exposeBridgeWorkspaceHint(tool) {
  ...
  const bridgeDescription = GENERATED_CONTRACT_MATERIALIZATION_TOOLS.has(tool?.name)
    ...
    : SEMANTIC_MODEL_REPAIR_TOOLS.has(tool?.name)
      ? " In installed Semaphor Agent Plugin runs, pass workspaceDir as a bridge-local auth/context hint; the bridge strips it before forwarding this server-owned semantic repair tool."
      : "";
  return {
    ...tool,
